...
- **Decisão**: a geração é **síncrona**, executada durante a interação do painel, com progresso reportado pelo circuito já existente (ADR-010) e o arquivo entregue como download ao final. Não há fila, worker, agendador nem armazenamento intermediário. Um **teto explícito de produtos por catálogo** é imposto e validado antes de iniciar, dimensionado para caber na meta de tempo de 3.3. A geração é serializada: uma de cada vez.
