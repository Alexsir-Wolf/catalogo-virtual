...
| T-07 (visibilidade da senha) | Concluído | 2026-09-23 | — | Botão de alternar visibilidade dentro do campo de senha, a pedido do dono. **Duas divergências registradas em vez de silenciadas:** o controle não constava da UI-03 — a SPEC-UI foi atualizada para incluí-lo — e ele exige JavaScript, ampliando a exceção às ADR-001 e ADR-010 já aberta para o estado de envio. A alternativa seria ida ao servidor carregando a senha em claro, o que seria pior que o problema. O campo nasce oculto, o ícone anuncia a ação disponível e não o estado, e `aria-pressed` e `aria-label` acompanham a troca. Um teste cobre a presença do controle e o estado inicial oculto. Suíte 50/52. **Conferência visual pendente:** a extensão do Chrome caiu durante a limpeza do ambiente anterior e o HTML foi verificado por requisição, não por olho |
